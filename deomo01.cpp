...
void creat_data()
{
	...
	for (double radian = 0.1; radian <= 2 * Pl; radian += 0.005)
	{
		x2 = 16 * pow(sin(radian), 3);
		y2 = 13 * cos(radian) - 5 *
			cos(2 * radian) - 2 * cos(3 * radian) - cos(4 * radian);
	...
	for (int frame = 0; frame < frames; ++frame)
	{
		...
		for (index = 0; index < points_size; ++index)
		{
			...
			solidcircle(screen_x(points[index].x)
				, screen_y(points[index].y), 1);
		}
	}
	for (double size = 17; size < 23;
		size += 0.3)
	{
		for (index = 0; index < quantity; ++index)
		{
			if ((creat_random(0, 100) / 100.0 > 0.6 && size >= 20) || (size < 20 && creat_random(0, 100) / 100.0> 0.95))
			{
				...
				if (size >= 20)
				{
					x = origin_points[index].x * size + creat_random(-frames* frames / 5 - 15, frames * frames / 5 + 15);
					y = origin_points[index].y * size + creat_random(-frames * frames / 5 - 15, frames * frames / 5 + 15);
				}
				else
				{
